...
class ProductController extends Controller
{
    ...
    public function update(Request $request, Product $product)
    {
        ...
        if ($oldStock > 10 && $validated['stock'] <= 10) {
    ...
    protected function sendStockAlert($product)
    {
        ...
        if (class_exists('\App\Models\Notification')) {
            \App\Models\Notification::create([
                ...
                'message' => "Produk {$product->name} memiliki stok rendah ({$product->stock} {$product->unit})",
                'data' => ['product_id' => $product->id],
            ]);
        }

        // Send WhatsApp alert if service is available
        if (class_exists('\App\Services\WhatsAppService')) {
            $whatsappService = app('\App\Services\WhatsAppService');
            $whatsappService->sendStockAlert($product);
        }
    }
